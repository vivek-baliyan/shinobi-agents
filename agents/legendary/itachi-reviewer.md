...
**EF Core lens:** relationships must be explicit (FK + `HasForeignKey` + delete behavior + decimal precision); any hand-created or hand-edited file under `Migrations/` is an automatic reject — migrations come only from `dotnet ef` (doctrine: API repo's CLAUDE.md).
